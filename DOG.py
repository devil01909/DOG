...
def reg():
    ...
    r = requests.get('https://github.com/devil01909/DOG/blob/main/Appruve.txt').text
    if to in r:
        ...
        os.system('xdg-open https://wa.me/+8801909576651')
        reg()
...
def moch_yayan():
    ...
    if pepek == '':
        ...
        time.sleep(1)
        os.system('rm -rf .memek.txt')
        jalan('\n %s[%s%s]%s berDOG menghapus token' % (N, H, N, H))
        exit()
    else:
            ...
            while True:
                ...
                def __yan__(ysc = (None, None)):
                    ...
                    if cin == '':
                        ...
                        print '\n [%s+%s] DOG OK file -> results/OK-%s-%s-%s.txt' % (O, N, ha, op, ta)
                        print ' [%s+%s] DOG CP file-> results/CP-%s-%s-%s.txt' % (O, N, ha, op, ta)
